...
$name=$email=$password=$phonenumber=$address=$city=$nationselect="";
$nameError=$emailError=$haserror=$passwordError=$phonenumberError=$addressError=$cityError=$nationselectError="";
...
if($haserror!=1){
    $mydb= new Model();
    $conobj= $mydb->OpenCon();
    $result=$mydb->AddStudent($conobj,"ma_register",$_REQUEST["Name"],$_REQUEST["Email"], $_REQUEST["Password"], $_REQUEST["PhoneNumber"],$nationselect);
    if($result===TRUE)
    {
